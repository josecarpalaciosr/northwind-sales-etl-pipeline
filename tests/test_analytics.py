...
from northwind_etl.load import TARGET_DATABASE_PATH
...
def execute_analytical_query(query_filename: str) -> pd.DataFrame:
    """Execute an analytical SQL query against the target database."""
...
def test_analytical_query_executes_and_returns_expected_columns(
    ...
    result = execute_analytical_query(query_filename)

    assert not result.empty
    assert expected_columns.issubset(result.columns)

def test_monthly_sales_uses_previous_month_for_growth() -> None:
...
def test_product_performance_ranks_and_category_shares() -> None:
    """Confirm product rankings and category revenue shares."""

    result = execute_analytical_query("product_performance.sql")
...
def test_country_performance_global_shares_and_ranking() -> None:
    """Confirm country revenue shares and ranking."""

    result = execute_analytical_query("country_performance.sql")
...
def test_employee_performance_metrics_are_valid() -> None:
    """Confirm employee performance percentages and ranking."""

    result = execute_analytical_query("employee_performance.sql")
...
def test_discount_performance_global_shares_and_rates() -> None:
    """Confirm discount-band shares and effective rates."""

    result = execute_analytical_query("discount_performance.sql")
